...
## 12. Предложения по развитию диалогов и Telegram-бота

### Главный пользовательский сценарий

`вопрос пользователя → ответ из проверенной базы знаний → сбор заявки при интересе → уведомление Дмитрию → передача человеку при сложном вопросе`

Ассистент должен помогать посетителю решить задачу, а не демонстрировать количество функций. Основное меню рекомендуется сделать коротким:

1. Новый диалог;
2. Услуги;
3. Цены;
4. Кейсы;
5. Связаться с Дмитрием.

### Лидогенерация

Бот последовательно запрашивает только необходимые данные:
...
После подтверждения бот создаёт заявку, отправляет уведомление на `dimitry8st@gmail.com` и сохраняет статус обращения. Владелец сайта не должен быть указан как заявитель, а поля формы должны оставаться пустыми до ввода пользователя.

### Передача человеку

Передача Дмитрию или оператору включается, если:

- пользователь прямо просит связаться с человеком;
- в базе знаний нет подтверждённого ответа;
- вопрос касается цены индивидуального проекта;
- пользователь недоволен двумя ответами подряд;
- требуется действие с оплатой, договором или персональными данными.

В уведомление передаются контакты, тема, краткое резюме и история диалога. Пользователю показывается понятный статус: «Заявка принята, Дмитрий ответит лично».

### Диалоговые настройки

Для каждого ассистента нужны:

- приветствие и короткое описание возможностей;
- кнопки быстрого выбора;
- готовые ответы на типовые команды;
- лимит уточняющих вопросов;
- правило отказа при отсутствии данных;
- единый стиль общения бренда ДИС;
- отдельная настройка web-чата и Telegram.

Автоматический перевод меню оставить для следующего этапа. Управление ботом по эмодзи или секретному слову не включать из-за риска случайных срабатываний.

### Защита

- ограничение частоты сообщений;
- антиспам и блокировка повторяющихся запросов;
- запрет раскрытия системного промпта, ключей и внутренних инструкций;
- маскирование телефонов, email и других персональных данных в логах;
- разграничение прав между чтением базы и выполнением действий;
- подтверждение перед оплатой, отправкой данных и внешними изменениями;
- хранение токенов только в переменных окружения;
- возможность быстро отозвать и заменить скомпрометированный токен.

### Журнал диалогов и качество

Для каждого обращения сохранять:
...
- оценку ответа пользователем;
- итоговый статус заявки.

Отдельно собирать список вопросов без хорошего ответа. Он станет основой для пополнения базы знаний и регулярной проверки качества.

### Монетизация — после проверки ценности

Монетизацию подключать после стабильной работы заявок и передачи человеку. Возможные варианты:

- фиксированная цена типового продукта;
- платная диагностика с зачётом суммы в проект;
- ежемесячная поддержка ассистента;
- оплата дополнительных каналов и интеграций.

AI не назначает цену самостоятельно. Платёжная ссылка создаётся только после выбора утверждённого предложения.

### Приоритет реализации

| Приоритет | Возможность | Результат |
|---|---|---|
| P0 | Заявка и уведомление | Дмитрий получает контакт и описание задачи |
| P0 | Передача человеку | Сложный диалог не теряется |
| P0 | Логи и вопросы без ответа | Видно, что улучшать в базе |
| P0 | Антиспам и защита секретов | Снижается риск злоупотреблений |
| P1 | Короткое меню и готовые ответы | Пользователю проще начать диалог |
| P1 | Web-чат и Telegram с общей историей | Единое обслуживание в двух каналах |
| P2 | Расписание и перевод | Расширение после пилота |
| P2 | Оплата | Подключение после проверки спроса |

### Критерии готовности функции

Функция считается готовой, если она протестирована на успешном и ошибочном сценариях, не раскрывает секреты, оставляет запись в журнале и имеет понятный способ передачи человеку.
