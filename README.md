...
Главная цель сайта — квалифицированные обращения на аудит, MVP и внедрение. Основной призыв ведёт в форму; Telegram временно открывает внутреннюю заглушку до запуска нового бота.
...
- единый каталог проверенных материалов `/materials/`;
- рубрики, теги, источники и связанные публикации;
- редакционные статусы, закрытый предпросмотр и безопасная публикация;
- обязательные источники, редактор, дата проверки и предупреждение для медицинских материалов;
- автоматический каталог клинических рекомендаций из официальных источников;
- блок новых материалов на главной и рекомендации по теме проекта.
...
| `GUIDELINES_SYNC_TOKEN` | Секрет Bearer для запуска синхронизации рекомендаций из n8n/cron |
| `CHAT_ENABLED` | Виджет FAQ. `0` выключает кнопку и `/chat/` |
| `OPENAI_API_KEY` | Опционально: формулировка ответа GPT. Без ключа — проверенный ответ из базы сайта |
| `OPENAI_MODEL` | Модель GPT для формулировки ответа; по умолчанию `gpt-4o-mini` |
...
Публичный каталог: `/clinical-guidelines/`. Российские карточки синхронизируются
с официальным JSON-методом Рубрикатора Минздрава, а международные адаптеры
передают метаданные через защищённый API. Полный порядок настройки n8n и формат
пакета описаны в [`docs/clinical-guidelines-sync.md`](docs/clinical-guidelines-sync.md).

Основной автоматический контур работает через GitHub Actions: раз в неделю
официальный реестр обновляет карточки и Markdown‑vault в `obsidian-vault/`.

Автоматически публикуются только библиографические карточки. AI-разбор,
практические выводы и любые медицинские интерпретации остаются в редакционном
контуре до проверки Степановым Д.А.
...
На публичных страницах — кнопка чата справа внизу. Дис сначала распознаёт
однозначные навигационные и безопасностные намерения, затем ищет по FAQ и
единому индексу страниц, статей, библиотеки, проектов и кейсов. GPT может
кратко сформулировать найденный ответ, но не используется как источник фактов.
Без ключа работает проверенный детерминированный ответ. Вопрос вне материалов
портала ведёт к честному отказу и форме связи. Админка виджет не показывает.

Проверяемые группы запросов: навигация по разделам, списки статей/проектов,
поиск по теме, услуги и цены, возможности Диса, Life-OS, KnightCat, чтение
библиотеки, медицинские ограничения, защита секретов и запрет действий от
имени владельца сайта. Регрессионный набор находится в
`tests/test_chat_intents.py`.
...
Кнопка «Обсудить внедрение» ведёт в форму с темой `faq`; до запуска нового бота Telegram ведёт на `/telegram/`.

## Чек-лист перед публикацией

- [ ] CTA ведёт в форму; Telegram показывает заглушку до запуска нового бота
