...
Счётчик посещений использует ту же базу данных. В подвале отображаются визиты
за сегодня, а в `/admin/` — сегодня, за 30 дней и всего (даты по Москве).
Один браузер считается один раз за 30 минут, переходы по страницам не
увеличивают показатель. IP-адреса в таблице счётчика не хранятся. Статистика
начинается с нуля после установки. Для сохранения истории после перезапуска
или обновления сервера настройте `DATABASE_URL` на постоянное хранилище:
SQLite на подключённом диске или PostgreSQL. Временный диск сервера может
стереть историю посещений.
В панели администратора даты «С» и «По» позволяют посмотреть визиты за любой
выбранный период до года включительно; обе указанные даты входят в расчёт.
...
### Автоответ на вопрос о стоимости

Посетитель может отдельно отметить в форме, что хочет получить ответ на email.
Если в заявке есть вопрос о цене и нет ссылок или SEO-рекламы, сайт отправляет
фиксированный русский текст с просьбой уточнить услугу, задачу и сроки.
Исходное сообщение не копируется в письмо. На один адрес отправляется не более
одного автоответа за календарную неделю; общий предел — 20 попыток за 24 часа.
Существующий предел формы (3 заявки в час с IP или адреса) действует и здесь.
Ошибки и количество отправленных/неотправленных писем за сутки видны в `/admin/`.

Для отправки настройте на сервере `SMTP_USERNAME`, `SMTP_PASSWORD` и при
необходимости `SMTP_FROM`; секреты храните только в серверном `.env`.
`INQUIRY_AUTO_REPLY_ENABLED=0` выключает автоответы после перезапуска сайта
без отключения приёма заявок. При ошибке SMTP повторная отправка не делается:
заявка остаётся в админке для ручного ответа.
